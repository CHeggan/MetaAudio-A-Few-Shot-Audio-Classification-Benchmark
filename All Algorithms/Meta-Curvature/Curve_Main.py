"""
Main  script for Meta-Curvature code
"""
###############################################################################
# IMPORTS
###############################################################################
...
def main(params, model, device, batch_fns, seed):
    ...
    # Start with first order with option to change later
    meta_curve = l2l.algorithms.GBML(
                module=model,
    ...
    meta_opt = optim.Adam(meta_curve.parameters(), lr=params['hyper']['meta_start_lr'])
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer=meta_opt,
                                        ...
                                        min_lr=params['hyper']['min_meta_lr'])
